...
vector<Complexe>veComp;
vector<vector<Complexe>>veCompImage;
...
void TransformationFourier(Image *in, Image *out)
{
	...
	for (int y = 0; y < in->height; y++)
	{
		for (int x = 0; x < in->width; x++)
		{
			...
			veComp.push_back(comp);//stocke la valeur de tel pixel sur la ligne y.
		}
		veCompImage.push_back(veComp);//stocke la totalite de la ligne y
		veComp.clear();
		
	}

	T = in->width;
cout<<"Taille = "<< in->width * in->height<<endl;
int tmp = 0;
	for (int x = 0; x < in->width; x++)
	{
		for (int y = 0; y < in->height; y++)
		{
			...
			for (int i = 0; i < in->height; i++)
			{
				...
				comp.Reel += (multComplexes(veCompImage.at(x).at(y), c)).Reel;
				comp.Imag += (multComplexes(veCompImage.at(x).at(y), c)).Imag;
				/*comp.Reel += (tComp[x][y].Reel * cos(2*PI*i*x/T));
				comp.Imag += (tComp[x][y].Imag * sin(2*PI*i*x/T));*/
			}
			ModifierPixel(out, x, y, /*EXPANSION DYNAMIQUE*/log( (sqrt(pow(getModule(comp.Reel, comp.Imag), 2))+1) ) );
			cout<<"TMP = "<<tmp<<endl;
		}
	}
	cout<<"FIN"<<endl;
