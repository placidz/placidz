...
{
	if (fabs(_in1.x - _in2.x) < 0.000001 || fabs(_in1.y - _in2.y) < 0.000001 || fabs(_in1.z - _in2.z) < 0.000001) return true;
	return false;
}
...
{
	mlVec3_SubVec(_e, _edge->head->pos, _edge->tail->pos);
}
...
int isInterior(mlVec3 _d, heEdge * _edge)
{
	...
	double w;
	mlVec3 v1, v2, v3, tmp;
	mlVec3_Copy(v1, _edge->tail);
	mlVec3_Copy(v2, _edge->head);
